...
test("Deve retornar status 200 e um JSON no POST",
async function(){
    const response = await request.put("/produtos/1").send({nome: "Banana nanica", preço: 18.00})
    expect(response.status).toBe(200)
    expect(response.headers['content-type'])
    .toMatch(/json/)
})

test("Deve retornar status 404 e um JSON no POST",
async function(){
...
test("Deve retornar status 404 e um 404 no DELETE",
async function(){
